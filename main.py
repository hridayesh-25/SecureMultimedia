...
# LOG RESULTS
# ---------------------------
def log_results(file_size, aes_enc, rsa_enc, sign_time,
                rsa_dec, aes_dec, verify_time):
    
    total_time = aes_enc + rsa_enc + sign_time + rsa_dec + aes_dec + verify_time

    non_aes_time = rsa_enc + sign_time + rsa_dec + verify_time

    overhead_percent = (non_aes_time / total_time) * 100
...
def run_mode(mode, repetitions=3):
    ...
    total_avg = total_time_total / repetitions

    non_aes_avg = rsa_enc_avg + rsa_dec_avg + sign_avg + verify_avg
    overhead_percent = (non_aes_avg / total_avg) * 100 if total_avg > 0 else 0
...
def log_comparison_results(mode, file_size, aes_enc, rsa_enc,
                           sign_time, rsa_dec, aes_dec,
                           verify_time, total_time, overhead):
